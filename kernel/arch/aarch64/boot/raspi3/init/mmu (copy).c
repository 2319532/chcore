...
void init_boot_pt(void)
{
        ...
        u64 kaddr = KERNEL_VADDR;
        u32 start = GET_L2_INDEX(kaddr);
        u32 end = start + PERIPHERAL_BASE/SIZE_2M;
        boot_ttbr1_l0[GET_L0_INDEX(kaddr)] = ((u64)boot_ttbr1_l1) | IS_TABLE
                                             | IS_VALID | NG;
	boot_ttbr1_l1[GET_L1_INDEX(kaddr)] = ((u64)boot_ttbr1_l2) | IS_TABLE
                                             | IS_VALID | NG;
        /* Step 2: map PHYSMEM_START ~ PERIPHERAL_BASE with 2MB granularity */
        
	for (u32 i=start; i<end;++i) {
                boot_ttbr1_l2[i] =
                        (PHYSMEM_START + i*SIZE_2M) /* low mem, va = pa */
                        | UXN /* Unprivileged execute never */
                        | ACCESSED /* Set access flag */
                        | NG /* Mark as not global */
                        | INNER_SHARABLE /* Sharebility */
        ...
        start = end;
        end = PHYSMEM_END/SIZE_2M;
	for (u32 i = start; i < end; ++i) {
		boot_ttbr1_l2[i] = 
		(PHYSMEM_START + i*SIZE_2M)
		|UXN
		|ACCESSED
		|NG
		|DEVICE_MEMORY
